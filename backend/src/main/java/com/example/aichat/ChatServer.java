...
public class ChatServer {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (basicConfig.getApiKeys() == null || basicConfig.getApiKeys().isEmpty() ||
            ...
            System.err.println("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
            // 可以决定是否退出或继续运行（继续运行会导致 API 调用失败）
            // System.exit(1); // 如果没有有效密钥，取消注释此行以退出程序
        }
        ...
        // 将加载的配置传递给处理器
        server.createContext("/api/chat", new ChatHandler(webConfig, basicConfig)); // <-- 传递配置
